...
class Compiler extends Tapable {
	...
	run(callback) {
		...
		// 此处会按顺序应用plugin，before-run -> run
		// 内部的this._plugins在Tapable的plugin方法中添加，后面可以看看在哪里添加的
		// before-run也是一个插件，在NodeEnvironmentPlugin.js当中，用于设置node的环境变量

		/*
			applyPluginsAsync流程
			1. 传入before-run, 从Tapable中取出this._plugin['before-run']的每个函数，依次执行（参数为Compiler,next)）
			2. 每次plugin函数执行，会执行传入的回调next，next通过闭包变量i记录plugins中的所有函数是否执行完，是的话执行applyPluginsAsync传入的回调

			P.S. 每个plugin都会有一个apply方法，这个方法会在第一次启动webpack的时候运行一遍，运行apply，在里面注册plugin，
			然后在类似applyPluginsXXX方法中来执行，在上面流程中，this._plugin['before-run']中的方法就是在各个插件提供的apply方法中注册的
			有一点值得疑惑，就是这种方法好像没办法看到plugin的名字，可能侧重点是可以按顺序执行
			PPS: 如果数组成员是个对象，并且是有继承过来的，可以看到父类的名字。。
		*/
